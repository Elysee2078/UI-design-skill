...
|---|---|---|---|
| [UI Skills](https://www.ui-skills.com/) | Design-engineering playbooks: baseline UI, frontend design, motion, accessibility, polish | **Integrate via CLI/MCP**. Prefer live `list_skills` / `get_skill` rather than copying stale text. MCP: `https://www.ui-skills.com/mcp` | Upstream GitHub `ibelick/ui-skills` is MIT at the time this catalog was prepared. Re-check before vendoring. |
| [coss.com/ui](https://coss.com/ui) | Accessible production React component reference from the Cal.com ecosystem | **Reference / selectively integrate** only from explicitly MIT upstream directories | Mixed license upstream: repository default AGPL-3.0; upstream identifies `apps/origin/` and `apps/ui/` as MIT. Verify exact path. |
...
| [ReUI](https://reui.io/) | Complex shadcn-compatible product UI: grids, calendars, filters, kanban, command surfaces and examples | **Integrate via MCP + registry**. Query component API/examples before implementation. MCP: `https://mcp.reui.io` | ReUI core/free repo is MIT; Pro blocks/templates/icons are commercial. Never bypass premium access. |
| [Kinetics](https://kinetics.colorion.co/) | Spring-based interaction and motion patterns with CSS/React examples | **Reference / integrate from MIT upstream as needed**, adapting to project tokens and reduced-motion policy | Upstream `ckissi/kinetics` advertises MIT. Verify current file/license before copying. |
...
| [Open Props](https://open-props.style/) | Reusable CSS design tokens for color, spacing, radii, shadows, easing, etc. | **Integrate as a package or selectively map tokens** when it reduces project debt | Check current package/repository license at install time; avoid copying an entire token universe when a smaller semantic layer is enough. |
...
| [bg.ibelick](https://bg.ibelick.com/) | Background pattern inspiration / snippets | **Reference by default** | A clear redistribution license was not established during preparation; verify before copying source snippets wholesale. |
| [Motion Primitives](https://motion-primitives.com/) | Reusable React motion primitives | **Integrate selectively** where a project already benefits from its motion stack | Upstream GitHub advertises MIT. Verify current license/dependencies before copying. |
